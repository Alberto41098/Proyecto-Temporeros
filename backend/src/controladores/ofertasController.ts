...
class OfertasController {
  ...
  public async readofertastrabajador(req: Request, res: Response) {
    const ofertas = await pool.query('SELECT * FROM ofertas where id_oferta in(select oferta_id from solicitudes where trabajador_id in ? )', [req.body.id]);
    res.json(ofertas);
  }
  public async readofertasempresa(req: Request, res: Response) {
    const ofertas = await pool.query('SELECT * FROM ofertas where empresa_id in ? )', [req.body.id]);
    res.json(ofertas);
  }
